...
int main(int argc, char** argv)
{
  ...
  printf("perf_detail: data_len:%d \n", perf_detail.data_len);
  for(int i=0; i<perf_detail.data_len; i++){
    printf("%c", perf_detail.perf_data[i]);
  }  
  ...
  gettimeofday(&start_time, NULL);
  post_process(outputs, io_num.n_output, height, width,
               box_conf_threshold, nms_threshold, scale_w, scale_h, out_zps, out_scales, &detect_result_group);
  // float_post_process(outputs, io_num.n_output, height, width,
  //              box_conf_threshold, nms_threshold, scale_w, scale_h, out_zps, out_scales, &detect_result_group);          
  gettimeofday(&stop_time, NULL);
  printf("postprocess use %f ms\n", (__get_us(stop_time) - __get_us(start_time)) / 1000);
  ...
  for (int i = 0; i < detect_result_group.count; i++) {
    ...
    putText(orig_img, text, cv::Point(x1, y1 -10), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 255));
  }
